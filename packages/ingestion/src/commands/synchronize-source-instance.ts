...
import { finalizeSynchronizationRun } from "../internal/commands/finalize-synchronization-run";
import { identifySynchronizationSource } from "../internal/commands/identify-synchronization-source";
import { markSynchronizationRunFailed } from "../internal/commands/mark-synchronization-run-failed";
import { recordConnectionFailure } from "../internal/commands/record-connection-failure";
import { recordConnectionResult } from "../internal/commands/record-connection-result";
...
import type { SynchronizationResult } from "../types";
...
export async function synchronizeSourceInstance(input: Readonly<{
  actionId: string;
  adapterKey: string;
  ...
  try {
    ...
    const status =
      failedConnectionCount === 0 && partialConnectionCount === 0
        ...
        : usefulConnectionCount === 0
          ? "failed"
          : "partial";
    await finalizeSynchronizationRun(runId, status, undefined, input.reporter);
    report(
    ...
    if (runId) {
      try {
        await finalizeSynchronizationRun(
          runId,
          "failed",
          failure,
          input.reporter,
