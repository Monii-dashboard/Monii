...
function accountCategory(account: PowensAccount): AccountCategory {
...
function accountPurpose(account: PowensAccount): AccountPurpose {
  if (account.usage === "PRIV") return "personal";
  if (account.usage === "ORGA") return "business";
  return "unknown";
}

function lifecycle(account: PowensAccount): ExternalAccountLifecycle {
...
function rawCurrency(account: PowensAccount): string | null {
  const value = account.currency?.id;
  return typeof value === "string" && value.trim() ? value : null;
}

function typeSupport(account: PowensAccount): ExternalAccountTypeSupport {
  const type = sourceType(account);
  if (type && UNSUPPORTED_TYPES.has(type)) return "known_unsupported";
  return accountCategory(account) === "unknown" ? "unrecognized" : "supported";
}
...
function normalizeAccount(
  ...
  return {
    ...
    identity: {
      ...
      reportedNameFingerprint: fingerprint(
        config.fingerprintKey,
        "reported_name",
    ...
    purpose: accountPurpose(account),
    rawCurrency: rawCurrency(account),
    reportedName: account.name || null,
    reportedType: sourceType(account),
...
export function createPowensFinancialSource(
  ...
  return {
    ...
    async listConnections(): Promise<readonly NormalizedExternalConnection[]> {
